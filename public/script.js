...
/**
   * drawFavoritePointsTable
   * @desc inserts the favorite points in a table ""
   * @param {*} results array of JSON with contains
   */
  function drawFavoriteTable(results) {
    ...
    for (var j = 0; j < results.length; j++) {
     ...
     cel1.innerHTML = results[j]._id;
     cel2.innerHTML = results[j].features[0].geometry.coordinates;
...
class HereAPI {
  constructor(){
    ...
    this.APP_ID = "mUl6LZ18uIkgsAxt-AVxzKbv_72KlcBbtdI0gOKklwE";
...
class MapInterface{
  ...
  addIcons(){
    this.busStopIcon = L.icon({
      iconUrl: 'public/src/icons/BusStopIcon.png',
      iconSize: [10, 10],
      iconAnchor: [5,5]
    });
  }
